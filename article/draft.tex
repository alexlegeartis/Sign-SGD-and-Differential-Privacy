...
We test our algorithm on the method of logistic regression for UCI Mushroom Dataset. The dataset consists of 6,449 training samples and 1,625 testing samples. Each sample has 112 features, and represents a mushroom either poisonous or edible. We apply different algorithms to train the logistic regression model. The algorithms are run on 10 workers, with training data distributed between the workers equally. We compare the cases when there is no noise and when the noise is normal (to-do: when we add Student's distribution, we'll have a heavy-tailed noise). The learning rate $\gamma = 0.02$, and there are 1000 iterations for each algorithm. The results are presented in \cref{fig:logreg}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{sign_sgd_dp.png}
    \caption{Logistic regression on UCI Mushroom Dataset.\newline Performance of GD, SignGD with majority voting, SGD, and SignSGD with majority voting.}
    \label{fig:logreg}
\end{figure}

As we see, Sign-SGD with majority voting has run correctly. It took about four times as many iterations as the standard SGD to converge. Nevertheless, it is only a moderate constant, so the algorithm proved to be applicable in practiceS as we estimated.
