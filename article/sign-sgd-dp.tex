...
\begin{theorem}[Theorem 11 from \cite{mironov2019renyidifferentialprivacysampled}]
    If $q \leq \frac{1}{5}$, $\sigma \geq 4$, and $\alpha$ satisfy  
    $$
    1 < \alpha \leq \frac{1}{2}\sigma^2 L - 2 \ln \sigma, \quad \text{and} \quad
    \alpha \leq \frac{\frac{1}{2}\sigma^2 L^2 - \ln 5 - 2 \ln \sigma}{L + \ln(q\alpha) + \frac{1}{2\sigma^2}},
    $$
    where  
    $$
    L = \ln\left(1 + \frac{1}{q(\alpha - 1)}\right),
    $$
    then SGM applied to a function of 2-sensitivity 1 satisfies $(\alpha, \varepsilon)$-RDP where  
    $$
    \varepsilon \triangleq \frac{2q^2 \alpha}{\sigma^2}.
    $$
\end{theorem}
