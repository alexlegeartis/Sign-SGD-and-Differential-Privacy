...
\begin{algorithm}[H]
    ...
    \label{alg:stochastic-sign-sgd}
    \textbf{Input:} Learning rate $\eta$, current hypothesis vector $w^{(t)}$, $M$ workers each with an independent gradient $g_m^{(t)}$, the 1-bit compressor $q(\cdot)$.\\
    \textbf{On server:}
    \begin{algorithmic}[1]
    \STATE Pull $q(g_m^{(t)})$ from each worker $m$.
    \STATE Compute $\tilde{g}^{(t)} = \text{sign}\left(\frac{1}{M} \sum_{m=1}^M q(g_m^{(t)})\right)$.
    \STATE Push $\tilde{g}^{(t)}$ to all the workers.
    \end{algorithmic}
    \textbf{On each worker:}
    \begin{algorithmic}[1]
    \STATE Update $w^{(t+1)} = w^{(t)} - \eta \tilde{g}^{(t)}$.
    \end{algorithmic}
    \end{algorithm}
...
\begin{definition}
For any given gradient $g_m^{(t)}$, the compressor $\text{dp-sign}$ outputs $\text{dp-sign}(g_m^{(t)}, \epsilon, \delta)$. The $i$-th entry of $\text{dp-sign}(g_m^{(t)}, \epsilon, \delta)$ is given by:
\[
\text{dp-sign}(g_m^{(t)}, \epsilon, \delta)_i =
\begin{cases} y
1, & \text{with probability } \Phi\left(\frac{(g_m^{(t)})_i}{\sigma}\right), \\
-1, & \text{with probability } 1 - \Phi\left(\frac{(g_m^{(t)})_i}{\sigma}\right),
\end{cases}
\]
where $\Phi$ is the cumulative distribution function (CDF) of the standard normal distribution; 
\[
\sigma = \frac{\Delta_2}{\epsilon \sqrt{2 \ln\left(\frac{1.25}{\delta}\right)}},
\]
where $\epsilon$ and $\delta$ are the differential privacy parameters, and $\Delta_2$ is the sensitivity measure.
\end{definition}
